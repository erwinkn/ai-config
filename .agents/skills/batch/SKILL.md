---
name: batch
description: Extract of Claude Code bundled `/batch` command (Claude Code 2.1.69). Research and plan a large-scale parallelizable change, then execute it across isolated worktree agents that each open a PR.
metadata:
...
This skill mirrors the bundled Claude `/batch` command behavior.

## Invocation Behavior

If invoked without an instruction, prompt for one:
...
If invoked outside a git repository, stop and report that `/batch` requires a git repo because it uses isolated git worktrees and PR creation.

If invoked with an instruction, execute the flow below.
