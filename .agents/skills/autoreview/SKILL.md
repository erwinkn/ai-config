...
- uses TruffleHog to gate only credentials added by the reviewed change, locally validates the full safe Git patch, recognizes synthetic fixture values tied to their credential field, reviews the bundle in one pass up to the aggregate prompt limit, and automatically uses complete bounded passes above it
