...
## Scope And Root-Cause Governor

Autoreview is a closeout gate, not permission to rewrite the task. The goal is not to make the reviewer stop complaining; it is to preserve or improve behavioral correctness and conceptual simplicity.
...
Do not interleave one finding with one patch. Complete the review pass first, then adjudicate the full finding set before editing:

1. **Verify evidence**: accept a finding only when it has a failing test, demonstrated execution path, violated documented invariant, dependency contract, or real user workflow. A new model opinion about an earlier model edit is not new evidence.
2. **Cluster by root cause**: several local findings may reflect one duplicated source of truth, wrong ownership boundary, or missing invariant.
3. **Find the canonical owner**: before adding another registry, validation/reconciliation pass, fallback, downstream special case, synchronization path, or compensating wrapper, prove that the canonical owner cannot be fixed directly.
4. **Choose one repair strategy per root cause**: prefer correcting the owner and deleting obsolete paths over layering guards around symptoms.
5. **Implement once and prove behavior**: run focused verification, then one blocker-only closeout review. Use a fresh model primarily to adjudicate finding validity and shared causes, not to generate another unbounded finding set.
...
Keep closeout and refactoring as separate lanes. Closeout remains P0-only, evidence-backed, and bounded to the existing task. When the correct repair changes architecture, open an explicit refactor task with behavior invariants first; allow replacement and deletion there, and judge success partly by reduced duplication, branches, concepts, and ownership boundaries.
...
After review-triggered edits, report the behavior evidence and commands run; files and non-test LOC before/after; abstractions, branches, registries, or config knobs added or removed; duplicated paths eliminated; and trade-offs moved to follow-up. These metrics are diagnostic rather than absolute gates, but any complexity increase must explain why the simpler canonical-owner fix was insufficient.
