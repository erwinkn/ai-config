...
```bash
skillx babysit-pr gh_pr_watch --pr auto --once
```

### Continuous watch (JSONL)

```bash
skillx babysit-pr gh_pr_watch --pr auto --watch
```

### Trigger flaky retry cycle (only when watcher indicates)

```bash
skillx babysit-pr gh_pr_watch --pr auto --retry-failed-now
```

### Explicit PR target

```bash
skillx babysit-pr gh_pr_watch --pr <number-or-url> --once
```
