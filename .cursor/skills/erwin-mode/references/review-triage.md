...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles review-bot or agentic review comments (Devin Review, Bugbot, Amp, and similar). The goal is not to ignore those comments by default. The goal is to stop treating every comment as a required code change.
...
- Confidence: candidate
- Skip when: A review bot flags an export, component, helper, or file as unused, and `gh stack view --json`, upper-stack diffs, or PR context shows it is used by a later PR in the stack.
