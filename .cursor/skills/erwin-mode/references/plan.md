...
Produce a phased implementation plan grounded in the **Principles** section of the `erwin-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `erwin-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.
...
- Prefer `subagent_type: "erwin-agent"`. `generalPurpose` is the fallback. Never use the built-in `plan` subagent_type; it ignores this skill.
- Pass `model:` from `skills/erwin-mode/references/models.md`: **how explorer** for code exploration, **judgment and prose** for judgment. If a slug is missing in this session, inherit-parent (omit Task `model`).
...
In the overview, name which erwin-mode non-negotiables the implementer must apply, by name:
...
- the **Babysit** playbook (`playbooks/babysit.md`) after the phase or full stack is built and the PR is ready for babysitting.
