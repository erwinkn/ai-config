...
function parseComment(value: unknown): T.ReviewComment {
  ...
  return {
    ...
    reviewDatabaseId: parseReviewDatabaseId(object.pullRequestReview),
  };
}
function parseReviewDatabaseId(value: unknown): number | null {
  if (value === undefined || value === null) return null;
  const review = record(value, "review comment.pullRequestReview");
  const id = review.databaseId;
  if (id === null || id === undefined) return null;
  if (typeof id === "number" && Number.isInteger(id) && id > 0) return id;
  missing("review comment.pullRequestReview.databaseId", id);
}
function isReviewBot(comment: T.ReviewComment | null): boolean {
  ...
  const authorHit =
    ...
    author.includes("amp-code") ||
    author === "amp" ||
    author.startsWith("amp[") ||
    author.includes("ampbot");
  const cursorAutomation =
    author === "cursor" &&
    [
      "bugbot",
  ...
  const bodyHit = ["devin review", "devin-review", "amp review"].some((token) =>
    body.includes(token)
  );
  return authorHit || cursorAutomation || bodyHit;
}
function passKey(comment: T.ReviewComment | null): string | null {
  ...
  for (const pattern of [
    ...
    /pr-review-job-([a-zA-Z0-9]+)/,
  ]) {
    const match = pattern.exec(comment.body);
    if (match?.[1]) return match[1];
  }
  return comment.reviewDatabaseId === null
    ? null
    : `review:${comment.reviewDatabaseId}`;
}
export function parseReviewThreadPage(value: unknown): {
  ...
  const nodes = list(threads.nodes, "reviewThreads.nodes");
  ...
  return { nodes, endCursor: page.hasNextPage && cursor ? cursor : null };
}
...
export function parseReviewThreadNodes(
  ...
  const passes = keys.size + (keyless ? 1 : 0);
  return threads
