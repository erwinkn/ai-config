...
it("treats Devin Review and Amp comments as review-bot threads", () => {
  const response = {
    data: {
      repository: {
        pullRequest: {
            ...
            nodes: [
              {
                ...
                comments: {
                  nodes: [
                    {
                      body: "Devin Review: unused export",
                      createdAt: "now",
                      path: "a.ts",
                      line: 1,
                      author: { login: "devin-bot" },
                    },
                  ],
                },
              },
              {
                ...
                comments: {
                  nodes: [
                    {
                      body: "amp review finding",
                      createdAt: "now",
                      path: "b.ts",
                      line: 2,
                      author: { login: "amp-code" },
                    },
                  ],
                },
              },
  ...
  const threads = parseReviewThreads(response);
  expect(threads.map((thread) => thread.isReviewBot)).toEqual([true, true]);
  expect(threads.map((thread) => thread.reviewBotPasses)).toEqual([1, 1]);
});

function reviewThreadNode({
  id,
  body,
  author,
  reviewDatabaseId,
  ...
  readonly reviewDatabaseId?: number;
      ...
      nodes: [
        {
          ...
          line: 1,
          author: { login: author },
          ...(reviewDatabaseId === undefined
            ? {}
            : { pullRequestReview: { databaseId: reviewDatabaseId } }),
        },
      ],
    },
  };
...
it("counts repeated Devin jobs, Amp reviews, and mixed Cursor plus keyless rounds", () => {
  const twoDevinJobs = parseReviewThreads({
    data: {
      repository: {
        pullRequest: {
          reviewThreads: {
            nodes: [
              reviewThreadNode({
                ...
                body: "pr-review-job-aaa111 finding one",
              }),
              reviewThreadNode({
                id: "d2",
                author: "devin-ai-integration",
                body: "pr-review-job-bbb222 finding two",
              }),
            ],
          },
        },
  ...
  expect(twoDevinJobs.map((thread) => thread.reviewBotPasses)).toEqual([2, 2]);

  const sameDevinJob = parseReviewThreads({
    data: {
      repository: {
        pullRequest: {
          reviewThreads: {
            nodes: [
              reviewThreadNode({
                id: "d1",
                author: "devin-ai-integration",
                body: "pr-review-job-aaa111 first finding",
              }),
              reviewThreadNode({
                id: "d2",
                author: "devin-ai-integration",
                body: "pr-review-job-aaa111 second finding",
              }),
            ],
          },
        },
  ...
  expect(sameDevinJob.map((thread) => thread.reviewBotPasses)).toEqual([1, 1]);

  const twoAmpReviews = parseReviewThreads({
    data: {
      repository: {
        pullRequest: {
          reviewThreads: {
            nodes: [
              reviewThreadNode({
                id: "a1",
                author: "amp-code",
                body: "amp review finding one",
                reviewDatabaseId: 11,
              }),
              reviewThreadNode({
                id: "a2",
                author: "amp-code",
                body: "amp review finding two",
                reviewDatabaseId: 22,
              }),
            ],
          },
  ...
  expect(twoAmpReviews.map((thread) => thread.reviewBotPasses)).toEqual([2, 2]);

  const mixed = parseReviewThreads({
    data: {
      repository: {
        pullRequest: {
          reviewThreads: {
            nodes: [
              ...
              reviewThreadNode({
                id: "d1",
                author: "devin-bot",
                body: "Devin Review: no job marker",
              }),
            ],
          },
        },
