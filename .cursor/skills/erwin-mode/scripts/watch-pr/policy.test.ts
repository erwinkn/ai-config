...
it("does not treat an ordinary example-tests check as a running review bot", async () => {
  const ordinary = await readSnapshot({
    reader: fakeReader({
      fastPath: { kind: "checks", checks: [pendingCheck("example tests")] },
    }),
    context: context(1),
    pendingHistory: "omit",
    allowDraft: false,
  });
  expect(ordinary.kind).toBe("open");
  if (ordinary.kind !== "open") throw new Error("expected open");
  expect(ordinary.reviewAutomationRunning).toBe(false);

  const amp = await readSnapshot({
    reader: fakeReader({
      fastPath: { kind: "checks", checks: [pendingCheck("Amp Review")] },
    }),
    context: context(1),
    pendingHistory: "omit",
    allowDraft: false,
  });
  expect(amp.kind).toBe("open");
  if (amp.kind !== "open") throw new Error("expected open");
  expect(amp.reviewAutomationRunning).toBe(true);
});
