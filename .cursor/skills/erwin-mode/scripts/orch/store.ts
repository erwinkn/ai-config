...
function parseFrontierState(state: string): FrontierPrState {
  ...
  if (normalized === "OPEN" || normalized === "QUEUED") {
    return "OPEN";
  }
  throw new UserError(
    `gh stack view --json has an unknown PR state: ${state}`
  );
}

function ghStackFrontier(repo: string): readonly {
  readonly pr: number;
  readonly state: FrontierPrState;
  readonly branches: string;
  ...
  const branches = (parsed as GhStackView).branches;
  ...
  for (const [index, item] of branches.entries()) {
    if (item === null || typeof item !== "object") {
      throw new UserError(
        `gh stack view --json has an unparseable branch at index ${index}`
      );
    }
    const branch = item as GhStackBranch;
    ...
    if (branch.pr === undefined) {
      throw new UserError(
        `gh stack view --json branch ${branch.name} has no pull request; resolve the frontier from a stacker clone with local tracking (gh stack checkout), not from gh stack link alone`
      );
    }
    const pr = Number(branch.pr.number);
    if (!Number.isSafeInteger(pr) || pr <= 0) {
      throw new UserError(
        `gh stack view --json branch ${branch.name} has an invalid PR number`
      );
    }
    if (typeof branch.pr.state !== "string") {
      throw new UserError(
        `gh stack view --json branch ${branch.name} has an invalid PR state`
      );
    }
    result.push({
      branches: branch.name,
      pr,
      state: parseFrontierState(branch.pr.state),
...
function resolveFrontier(repo: string): readonly FrontierPr[] {
  return ghStackFrontier(repo).map((row) => ({
    ...
    state: row.state,
    sha:
      row.head !== null && /^[0-9a-f]{40,64}$/i.test(row.head)
        ? row.head
        : branchSha({ branch: row.branches, repo }),
  }));
}
