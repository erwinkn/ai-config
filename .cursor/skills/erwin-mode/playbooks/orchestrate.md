...
FORBIDDEN    no gt, no gh stack submit/sync/rebase, no pulling sibling branches, no force-push, no fixes outside scope, plus unit-specific bans
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from `gh stack view --json` after every merge and stack mutation because GitHub base refs drift mid-restack while stack tracking is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it where the stacker knows the stack, normally the stacker's clone; a checkout whose stack metadata never saw the submits reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may run `gh stack` topology commands (`link`, `submit`, `sync`, `rebase`, `merge`), serialized within its stack; record the holder in the standing orders. Restacks run in cloud; a local restack at this scale takes the laptop down. Cross-fork stacks are not supported.
- Workers never rebase and never run `gh stack submit`, `gh stack sync`, or `gh stack rebase`. They never pull sibling branches. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than restacking.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the cloud agent's status in the harness dashboard. Transcript mtime is not liveness.
