...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the **bug-fix** row in `skills/erwin-mode/references/models.md` for this harness (inherit-parent if that slug is missing) with a specific scope; review the diff.
