...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using the **perf-issue** row in `skills/erwin-mode/references/models.md` for this harness (inherit-parent if that slug is missing); review the diff. Capture a post-fix trace.
