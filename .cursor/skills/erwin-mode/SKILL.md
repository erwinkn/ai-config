---
name: Erwin Mode
description: erwin's agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Use for erwin, /erwin-mode, or requests to work in this style.
disable-model-invocation: true
mode: true
icon: crown
color: yellow
reminder: New task? Playbook match or rigor needed -> apply /erwin-mode. Casual turn or user opts out -> don't.
---
...
- Shipping UI / IDE / CLI → the **control-cli** skill (CLIs and TUIs) or the **control-ui** skill (browser / Electron / web UIs). For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
- Stacked PRs → GitHub native `gh stack` plus the GitHub API. Same-repo only; cross-fork stacks are not supported. Cloud owners push their own branch and report the intended parent; the root owns topology via `gh stack link`. Never tell a cloud agent to pull others' branches into `gh stack submit`.
- A review bot or agentic review commented (Devin Review, Bugbot, Amp, or similar) → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/review-triage.md`. Bakeoffs stay on the **interrogate** skill.
...
**Use `subagent_type: "erwin-agent"` for any subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers). `/erwin-mode` and `erwin-agent` route through the same wrapper. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own `subagent_type` for diverse-model review; respect what the skill prescribes, don't override to `erwin-agent`.

**Defaults for every `Task` call.** `run_in_background: true`, agent mode (readonly strips MCP), file pointers not inlined context, explicit model per role. Read `skills/erwin-mode/references/models.md`. Detect the harness (Cursor / Claude Code / Codex) and take that column. Cloud-only on Cursor. No Composer. Cloud agents are one model each; mixed panels are multiple Task agents from the coordinator, one subagent per list entry. If a slug is missing in this session, inherit-parent: omit Task `model`. `inherit-parent` and `auto` always mean omit Task `model` and run on the parent chat model.

Code delegates tier by difficulty. Use the **feature, refactoring** row for ordinary code, **bug-fix** / **perf-issue** / **hillclimb** for those playbooks, **judgment and prose** for prose and judgment, **hardest tasks** for cross-cutting design, gnarly concurrency, or a subtle algorithm. Trivial mechanical edits use **feature, refactoring**. Live QA / drive-the-app delegates use **QA tester**. Reflect tooling uses **reflect tooling**. The routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`) name their own rows in that table.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run with `gh stack merge`. `playbooks/shipping.md`.
