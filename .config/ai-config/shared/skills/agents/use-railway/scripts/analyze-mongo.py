...
def run_mongosh_query(service: str, js_expr: str, timeout: int = 30) -> Tuple[int, str, str]:
    ...
    command = (
        f'''bash +H -c 'mongosh --host localhost --port 27017 '''
        f'''--username "$MONGOUSER" --password "$MONGOPASSWORD" '''
        f'''--authenticationDatabase admin --quiet --norc '''
        f'''--eval "EJSON.stringify({escaped})"' '''
    )
